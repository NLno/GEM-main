...
def get_smiles_list_pkl():
    dataset_df = pd.read_csv(current_path+"./data/lipo/raw/dataset.csv")
    smiles_list = dataset_df["smiles"].tolist()
    smiles_list = list(set(smiles_list))
    print(len(smiles_list))
    pkl.dump(smiles_list, open(current_path+'./data/lipo/intermediate/smiles_list.pkl', 'wb'))
...
def calculate_3D_structure_(smiles_list):
    ...
    while True:
        ...
        try:
            ...
            mutex.acquire()
            with open(current_path+'./data/lipo/result/invalid_smiles.txt', 'a') as f:
                f.write(str(smiles) + '\n')
            mutex.release()
            continue
...
def calculate_3D_structure():
    smiles_list_unique = pkl.load(open(current_path+'./data/lipo/intermediate/smiles_list.pkl', 'rb'))
    global smiles_to_graph_dict
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_graph_dict, open(current_path+'./data/lipo/intermediate/smiles_to_graph_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_graph_dict))

def construct_data_list():
    dataset_df = pd.read_csv(current_path+"./data/lipo/raw/dataset.csv")
    smiles_to_graph_dict:dict = pkl.load(open(current_path+'./data/lipo/intermediate/smiles_to_graph_dict.pkl','rb'))
    ...
    pkl.dump(data_list_train, open(current_path+'./data/lipo/intermediate/data_list_train.pkl', 'wb'))
    pkl.dump(data_list_validate, open(current_path+'./data/lipo/intermediate/data_list_validate.pkl', 'wb'))
    pkl.dump(data_list_test, open(current_path+'./data/lipo/intermediate/data_list_test.pkl', 'wb'))
...
def get_data_loader(batch_size):
    ...
    data_list_validate = pkl.load(open(current_path+'./data/lipo/intermediate/data_list_validate.pkl', 'rb'))
    data_list_test = pkl.load(open(current_path+'./data/lipo/intermediate/data_list_test.pkl', 'rb'))
...
def trial(model_version):
    ...
    for epoch in range(800):
        ...
        if metric_validate['rmse'] < current_best_metric:
            ...
            pdl.save(model.state_dict(), current_path+"./weight/lipo/" + model_version + ".pkl")
        print("=========================================================")
        print("Epoch", epoch)
